...
void load_ycsb_inserts(std::vector<uint64_t>& vect) {
  ...
  for (int trial = 0; trial < 6; trial++) {
    ...
    concurrent_map.map_range_length(1, vect.size(), [&sum_key_range, &sum_val_range]([[maybe_unused]] auto el) {
...
//   return;
  ...
  for (int trial = 0; trial < 6; trial++) {
    ...
    concurrent_map.map_range_length(1, data.size(), [&sum_key_range, &sum_val_range]([[maybe_unused]] auto el) {
                  sum_key_range += el.first;
                  sum_val_range += el.second;
                });
...
int main(int argc, char *argv[]) {
  std::vector<uint64_t> test_arr;
  load_ycsb_inserts(test_arr);
  return 0;
}