...
test_concurrent_microbenchmarks_map(uint64_t max_size, uint64_t NUM_QUERIES, std::seed_seq &seed, bool write_csv) {
  printf("\nRunning plain btree with internal bytes = %lu, leaf bytes = %lu\n",internal_bytes, leaf_bytes);
  ...
  if (!write_csv) { 
    ...
    outfile << "plain, " << internal_bytes << ", " <<  leaf_bytes << ", " <<  max_size << ", " <<  insert_time << ", " <<  NUM_QUERIES << ", " <<  find_time << ", \n";
    outfile.close();
    outfile.open("range_queries.csv", std::ios_base::app); 
    for (size_t i = 0; i < num_query_sizes.size(); i ++ ) {
      outfile << "plain, " << internal_bytes << ", " <<  leaf_bytes << ", " <<  max_size << ", " <<  NUM_QUERIES << ", " <<  num_query_sizes[i] << ", " <<  unsorted_range_query_times_by_size[i] << ", " <<  sorted_range_query_times_by_size[i] << ", \n";
    }
    outfile.close();
  }
  return true;
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < trials; i++) {
    ...
    bool correct = test_concurrent_microbenchmarks_map<unsigned long, 256, 256>(n, num_queries, seed, write_csv);
    correct = test_concurrent_microbenchmarks_map<unsigned long, 512, 512>(n, num_queries, seed, write_csv);
    correct = test_concurrent_microbenchmarks_map<unsigned long, 1024, 1024>(n, num_queries, seed, write_csv);
    correct = test_concurrent_microbenchmarks_map<unsigned long, 2048, 2048>(n, num_queries, seed, write_csv);
